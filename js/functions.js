...
function generarTableroHTML() {
    ...
    tableroHTML.style.background = "#F0F0F0"
}
    ...
    for (let f = 0; f < filas; f++) {
        for (let c = 0; c < columnas; c++) {
            ...
            if (tablero[c][f].estado == "descubierto") {
                ...
                celda.style.background = "#AAAAFF"
                switch (tablero[c][f].valor) {
                    case -1:
                        celda.innerHTML = `<i class="fas fa-venus"></i>`
                        celda.style.color = "#FF00FF"
            ...
            if (tablero[c][f].estado == "marcado") {
                celda.innerHTML = `<i class="fas fa-exclamation"></i>`
...
function actualizarPanelMinas() {
    ...
    panel.innerHTML = `${minas - marcas} mujeres`
}
...
function verificarGanador() {
    ...
    */
    ...
    let tableroHTML = document.getElementById("tablero")
    tableroHTML.style.background = "#55FF55"
    enJuego = false
}
    ...
    for (let f = 0; f < filas; f++) {
        for (let c = 0; c < columnas; c++) {
            ...
            if (tablero[c][f].valor == -1) {
                if (tablero[c][f].estado == `descubierto`) {
                    let tableroHTML = document.getElementById("tablero")
                    tableroHTML.style.background = "#FF00FF"
                    enJuego = false
                    let showName = Math.floor(Math.random() * nameGirls.length)
                    alert(`${nameGirls[showName]} asesino a leito`);
                }
            }
        }
    }
